...
def test_get_notifications(client, student_token):
    ...
    assert isinstance(response.json["notifications"], list), "Notifications should be a list"
...
def test_get_notification_detail(client, student_token, user_notification):
    ...
    assert user_notification.read_at is not None, "Notification should be marked as read"
...
def test_mark_all_notifications_as_read(client, student_token, user_notification):
    ...
    user_notification1 = UserNotifications(
        notification_id=additional_notification1.id,
        user_id=student.id,
        read_at=None
    )
    user_notification2 = UserNotifications(
        notification_id=additional_notification2.id,
        user_id=student.id,
        read_at=None
    )
    ...
    unread_notifications = UserNotifications.query.filter_by(user_id=student.id, read_at=None).all()
...
def test_get_notification_preferences(client, student_token):
    ...
    assert response.json["email_deadline_notifications"] == preferences.email_deadline_notifications
    assert response.json["in_app_deadline_notifications"] == preferences.in_app_deadline_notifications
    assert response.json["email_feedback_notifications"] == preferences.email_feedback_notifications
    assert response.json["in_app_feedback_notifications"] == preferences.in_app_feedback_notifications
...
#     assert response.content_type == "application/json", "Expected response to be in JSON format"
...
#     # Debug the response content to check for any issues
#     print("Response Data:", response.data)  # You can remove this after debugging

#     # Assert the response is correct
#     assert response.status_code == 404, "Notification preferences should not be found"
#     assert response_json.get("message") == "Notification preferences not found.", f"Unexpected message: {response_json.get('message')}"
...
def test_get_milestone_details(client, student_token, milestone_id):
    ...
    assert response.json["description"] == "This is a test milestone", "Unexpected milestone description"
...
def submission(client, student_token, milestone, team):
    ...
    file_path = os.path.join(file_dir, f"Milestone{milestone.id}_Task{task_id}_Team{team.name}.pdf")
